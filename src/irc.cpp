...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BambooTEST0\r");
            Send(hSocket, "WHO #BambooTEST0\r");
        } else {
            // randomly join #Bamboo00-#Bamboo99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Bamboo%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Bamboo%02d\r", channel_number).c_str());
        }
